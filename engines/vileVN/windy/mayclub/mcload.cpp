...
MCLoad::MCLoad(Mayclub *Engine) : DialogBase(Engine,true){
	// Load main menu graphics
	SDL_Surface *sback=Engine->LoadImage("load.bet");
	...
	if(sback && smarker && mmarker && skeywait && mkeywait){
		...
		for(int i=0;i<4;i++){
			SDL_Rect rdest={1,117+(80*i),317,78};
...
bool MCLoad::FocusEnter(Widget *Object){
	if(Object==w_pages[0] ||
	   Object==w_pages[1] ||
	   Object==w_pages[2] ||
	   Object==w_pages[3] ||
...
bool MCLoad::FocusLeave(Widget *Object){
	w_hover->SetVisible(false);
	return false;
}

bool MCLoad::InputOk(Widget *Object){
	bool retval=false;
	if(Object==w_exit){
		// Close dialog
		...
		if(((Mayclub*)engine)->EventLoad(index)){
			engine->DestroyWidget(this);
			engine->SetTransition();
		}
		retval=true;
